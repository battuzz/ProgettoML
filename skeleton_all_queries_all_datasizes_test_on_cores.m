...
if OUTPUT_LATEX
	flatex = fopen('latex_output/nonlinear_all_features_test_on_ncores.tex', 'w');
	...
	for datasize_id = 1:length(DATASIZES)
		...
		while (length(TRAINING_TO_DO) > 0) & (length(TESTING_TO_DO) > 0)
			try
				...
				TABLE_LABEL = cstrcat('tab:', 'all_nonlinear_', QUERY, '_', DATASIZE, '_', strjoin(strsplit(int2str(TESTING_CORES)), '_'));
				PLOT_LABEL = cstrcat('fig:', 'all_nonlinear_', QUERY, '_', DATASIZE, '_', strjoin(strsplit(int2str(TESTING_CORES)), '_'));

				% OUTPUT_FOLDER = strcat('output/', QUERY, '_ALL_FEATURES/');
				OUTPUT_FOLDER = strcat('output/', QUERY, '_', DATASIZE, '_ALL_NONLINEAR_', strjoin(strsplit(int2str(TESTING_CORES)), '_'), '/');
