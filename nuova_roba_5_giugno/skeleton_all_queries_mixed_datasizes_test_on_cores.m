...
if OUTPUT_LATEX
	flatex = fopen('latex_output/aggregated_nonlinear_advanced_test_on_ncores_v2_bestmodels.tex', 'w');
	fprintf(flatex, cstrcat('\\newpage\n', ...
							'\\section{Aggregated, $ncores^{-1}$, all the features with $nmap/ncores$ and $nreduce/ncores$, test on ncores}\n'));
end
...
for query_id = 1:length(QUERIES)
	...
	while (length(TRAINING_TO_DO) > 0) & (length(TESTING_TO_DO) > 0)
		try
			...
			TABLE_CAPTION = cstrcat('Results for ', QUERY, ', testing on ', strjoin(strsplit(int2str(TESTING_CORES)), ', '), ' cores');
			PLOT_CAPTION = cstrcat('Completion time vs ncores for ', QUERY, ', testing on ',  strjoin(strsplit(int2str(TESTING_CORES)), ', '), ' cores');
			TABLE_LABEL = cstrcat('tab:', 'aggr_advanced_nonlinear_', QUERY, '_', strjoin(strsplit(int2str(TESTING_CORES)), '_'));
			PLOT_LABEL = cstrcat('fig:', 'aggr_advanced_nonlinear_', QUERY, '_', strjoin(strsplit(int2str(TESTING_CORES)), '_'));

			% OUTPUT_FOLDER = strcat('output/', QUERY, '_ALL_FEATURES/');
			OUTPUT_FOLDER = strcat('output/', QUERY, '_ADVANCED_NONLINEAR_', strjoin(strsplit(int2str(TESTING_CORES)), '_'), '/');
