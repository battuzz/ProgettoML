...
FEATURES = [1:10, 12, 13];	%% DO NOT CHANGE
ADVANCED_FEATURES = true; 	% nmap/ncores, nreduce/ncores
...
if OUTPUT_LATEX
	flatex = fopen('latex_output/aggregated_nonlinear_advanced_shuffled.tex', 'w');
	fprintf(flatex, cstrcat('\\newpage\n', ...
							'\\section{Aggregated, $ncores^{-1}$, all the features with $nmap/ncores$ and $nreduce/ncores$, shuffled}\n'));
end
...
for query_id = 1:length(QUERIES)
	...
	TABLE_CAPTION = cstrcat('Results for ', QUERY);
	PLOT_CAPTION = cstrcat('Completion time vs ncores for query ', QUERY);
	TABLE_LABEL = cstrcat('tab:', 'advanced_nonlinear_shuffled_', QUERY);
	PLOT_LABEL = cstrcat('fig:', 'advanced_nonlinear_shuffled_', QUERY);

	% OUTPUT_FOLDER = strcat('output/', QUERY, '_ALL_FEATURES/');
	OUTPUT_FOLDER = strcat('output/', QUERY, '_ADVANCED_NONLINEAR_SHUFFLED/');
	...
	if OUTPUT_LATEX
		...
		fprintf(flatex_plot, cstrcat('\n\\begin {figure}[hbtp]\n', ...
									'\\centering\n', ...
									'\\includegraphics[width=\\textwidth]{', OUTPUT_FOLDER, 'plot_', QUERY, OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{2}, '}\n', ...
		...
		if BEST_MODELS
			fprintf(flatex_plot_bestmodels, cstrcat('\n\\begin {figure}[hbtp]\n', ...
													'\\centering\n', ...
													'\\includegraphics[width=\\textwidth]{', OUTPUT_FOLDER, 'plot_', QUERY, '_bestmodels', OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{2}, '}\n', ...
													'\\caption{', PLOT_CAPTION, '}\n', ...
													'\\label{', PLOT_LABEL, '}\n', ...
													'\\end {figure}\n'));	
	...
	if ALL_THE_PLOTS
		for col = 1:M
			...
			if (N_CORES_INVERSE & ismember(13, FEATURES) & (col == 11))
			...
			if LINEAR_REGRESSION
				...
				x = x_denorm;
				y = ylin;
				save(cstrcat(OUTPUT_FOLDER, 'Linear Regression.mat'), 'x', 'y', 'QUERY');
			...
			for index = 1:length(MODELS_CHOSEN)
				...
				x = x_denorm;
				y = ysvr;
				save(cstrcat(OUTPUT_FOLDER, SVR_DESCRIPTIONS{index}, '.mat'), 'x', 'y', 'QUERY');

			end

			% Plot the mean of the test values (for nCores)
			% if (TEST_ON_CORES & ismember(13, FEATURES) & (col == 11))
			...
			if SAVE_DATA
				...
				% file_location = strrep(strcat(OUTPUT_FOLDER, 'plot_', QUERY,  FEATURES_DESCRIPTIONS{col}, OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{2}), ' ', '');
				print(OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{1}, file_location);
			end

			if SAVE_DATA & ismember(13, FEATURES) & (col == 11)
				file_location = strrep(strcat(OUTPUT_FOLDER, 'plot_', QUERY, OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{2}), ' ', '');
				print(OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{1}, file_location);
			end
	...
	if BEST_MODELS
		...
		if ALL_THE_PLOTS
			...
			for col = 1:M
				...
				if SAVE_DATA & ismember(13, FEATURES) & (col == 11)
					file_location = strrep(strcat(OUTPUT_FOLDER, 'plot_', QUERY, '_bestmodels', OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{2}), ' ', '');
					print(OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{1}, file_location);
				end
