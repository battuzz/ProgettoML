...
if OUTPUT_LATEX
	flatex = fopen('output/query_comparison.tex', 'w');
...
% 	QUERY = QUERIES{query_id};
	...
	fprintf(flatex, cstrcat('\\subsection{R3, R4 $\\rightarrow$ R2 (Fast - Slow)}\n'));

	for datasize_id = 1:length(DATASIZES)

		DATASIZE = DATASIZES{datasize_id};

		fprintf(flatex, cstrcat('\\subsubsection{', strjoin(TRAIN_QUERIES, ','), ' $\\rightarrow$ ', strjoin(TEST_QUERIES, ','), ' --- Datasize ', DATASIZE, 'GB}\n'));
		...
		TRAIN_DATA_LOCATION = {};
		TEST_DATA_LOCATION = {};
		for query_id = 1:length(TRAIN_QUERIES)
			TRAIN_DATA_LOCATION{end + 1} = strcat(TRAIN_QUERIES{query_id}, '/Datasize/', DATASIZE);
		end
		for query_id = 1:length(TEST_QUERIES)
			TEST_DATA_LOCATION{end + 1} = strcat(TEST_QUERIES{query_id}, '/Datasize/', DATASIZE);
		end
		

		TABLE_CAPTION = cstrcat('Results for ', strjoin(TRAIN_QUERIES, ','), ' $\\rightarrow$ ', strjoin(TEST_QUERIES, ','), '-', DATASIZE);
		PLOT_CAPTION = cstrcat('Completion time vs ncores for: ',strjoin(TRAIN_QUERIES, ','), ' $\\rightarrow$ ', strjoin(TEST_QUERIES, ','), ' with datasize ', DATASIZE, 'GB');
		TABLE_LABEL = cstrcat('tab:', 'coreonly_linear_', strjoin(TRAIN_QUERIES, ','), '_', strjoin(TEST_QUERIES, ','), '_', DATASIZE);
		PLOT_LABEL = cstrcat('fig:', 'coreonly_linear_', strjoin(TRAIN_QUERIES, ','), '_', strjoin(TEST_QUERIES, ','), '_', DATASIZE);

		% OUTPUT_FOLDER = strcat('output/', QUERY, '_ALL_FEATURES/');
		OUTPUT_FOLDER = strcat('output/', strjoin(TRAIN_QUERIES, '_'), '_VS_', strjoin(TEST_QUERIES, '_'), '_', DATASIZE, '_ALL_FEATURES/');
		...
		if OUTPUT_LATEX
			...
			fprintf(flatex_plot, cstrcat('\n\\begin {figure}[hbtp]\n', ...
										'\\centering\n', ...
										'\\includegraphics[width=\\textwidth]{', OUTPUT_FOLDER, 'plot_', strjoin(TRAIN_QUERIES, '_'), '_VS_', strjoin(TEST_QUERIES, '_'), '_', DATASIZE, OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{2}, '}\n', ...
			...
			if BEST_MODELS
				fprintf(flatex_plot_bestmodels, cstrcat('\n\\begin {figure}[hbtp]\n', ...
														'\\centering\n', ...
														'\\includegraphics[width=\\textwidth]{', OUTPUT_FOLDER, 'plot_', strjoin(TRAIN_QUERIES, '_'), '_VS_', strjoin(TEST_QUERIES, '_'), '_', DATASIZE, '_bestmodels', OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{2}, '}\n', ...
														'\\caption{', PLOT_CAPTION, '}\n', ...
														'\\label{', PLOT_LABEL, '}\n', ...
														'\\end {figure}\n'));	
		...
		if ALL_THE_PLOTS
			for col = 1:M
				...
				if LINEAR_REGRESSION
					...
					x = x_denorm;
					y = ylin;
					save(cstrcat(OUTPUT_FOLDER, 'Linear Regression.mat'), 'x', 'y', 'TRAIN_QUERIES', 'TRAIN_QUERIES', 'DATASIZE');
				...
				for index = 1:length(MODELS_CHOSEN)
					...
					x = x_denorm;
					y = ysvr;
					save(cstrcat(OUTPUT_FOLDER, SVR_DESCRIPTIONS{index}, '.mat'), 'x', 'y', 'TRAIN_QUERIES', 'TEST_QUERIES', 'DATASIZE');
				...
				if SAVE_DATA & ismember(13, FEATURES) & (col == M)
					file_location = strrep(strcat(OUTPUT_FOLDER, 'plot_', strjoin(TRAIN_QUERIES, '_') , '_VS_', strjoin(TEST_QUERIES, '_') , '_', DATASIZE, OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{2}), ' ', '');
					print(OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{1}, file_location);
				end
			...
			if LINEAR_REGRESSION
				...
				x = x_denorm;
				y = ylin;
				save(cstrcat(OUTPUT_FOLDER, 'Linear Regression.mat'), 'x', 'y', 'TRAIN_QUERIES', 'TEST_QUERIES', 'DATASIZE');
		...
		if BEST_MODELS
			...
			if ALL_THE_PLOTS
				...
				for col = 1:M
					...
					if SAVE_DATA & ismember(13, FEATURES) & (col == M)
						file_location = strrep(strcat(OUTPUT_FOLDER, 'plot_', strjoin(TRAIN_QUERIES, '_') , '_VS_', strjoin(TEST_QUERIES, '_') , '_', DATASIZE, '_bestmodels', OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{2}), ' ', '');
						print(OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{1}, file_location);
					end
