...
TRAIN_QUERIES = {'R3', 'R4'};
TEST_QUERIES = {'R2'};
TEST_ID = 'query_comp_006';  %% that'll do
...
TRAIN_DATA_LOCATION = TRAIN_QUERIES;

%% List of all directories with test data (leave {} if test data equals train data)
% TEST_DATA_LOCATION = {'Query R/R1/Core/120'};
TEST_DATA_LOCATION = TEST_QUERIES;

OUTPUT_LATEX = true;

TABLE_CAPTION = 'Results for ';
for i = 1:length(TRAIN_QUERIES)
	TABLE_CAPTION = cstrcat(TABLE_CAPTION, TRAIN_QUERIES{i}, ' ');
end
TABLE_CAPTION = cstrcat(TABLE_CAPTION, '-> ');
for i = 1:length(TEST_QUERIES)
	TABLE_CAPTION = cstrcat(TABLE_CAPTION, TEST_QUERIES{i}, ' ');
end

PLOT_CAPTION = 'Completion time vs ncores';
TABLE_LABEL = cstrcat('tab:', TEST_ID);
PLOT_LABEL = cstrcat('fig:', TEST_ID);
...
for i = 1:length(TRAIN_QUERIES)
	OUTPUT_FOLDER = cstrcat(OUTPUT_FOLDER, TRAIN_QUERIES{i}, '_');
end
OUTPUT_FOLDER = cstrcat(OUTPUT_FOLDER, 'TO_');
for i = 1:length(TEST_QUERIES)
	OUTPUT_FOLDER = cstrcat(OUTPUT_FOLDER, TEST_QUERIES{i}, '_');
end
OUTPUT_FOLDER = cstrcat(OUTPUT_FOLDER, 'ALL/');
...
if OUTPUT_LATEX
	...
	fprintf(flatex, cstrcat('\n\\begin {figure}[hbtp]\n', ...
							'\\centering\n', ...
							'\\includegraphics[width=\\textwidth]{', OUTPUT_FOLDER, 'plot_', TEST_ID, '_bestmodels', OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{2}, '}\n', ...
...
for col = 1:M
	...
	if LINEAR_REGRESSION
		...
		x = x_denorm;
		y = ylin;
		save(cstrcat(OUTPUT_FOLDER, 'Linear Regression.mat'), 'x', 'y', 'TRAIN_QUERIES', 'TEST_QUERIES');
	...
	for index = 1:length(MODELS_CHOSEN)
		...
		x = x_denorm;
		y = ysvr;
		save(cstrcat(OUTPUT_FOLDER, SVR_DESCRIPTIONS{index}, '.mat'), 'x', 'y', 'TRAIN_QUERIES', 'TEST_QUERIES');
	...
	if SAVE_DATA & ismember(13, FEATURES) & (col == M)
		file_location = strrep(strcat(OUTPUT_FOLDER, 'plot_', TEST_ID, OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{2}), ' ', '');
		print(OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{1}, file_location);
	end
...
if BEST_MODELS
	for col = 1:M
		...
		if SAVE_DATA & ismember(13, FEATURES) & (col == M)
			file_location = strrep(strcat(OUTPUT_FOLDER, 'plot_', TEST_ID, '_bestmodels', OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{2}), ' ', '');
			print(OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{1}, file_location);
		end
