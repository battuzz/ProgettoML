...
if OUTPUT_LATEX
	flatex = fopen('latex_output/fixed_datasize_only_linear_ncores.tex', 'w');
...
for query_id = 1:length(QUERIES)
	...
	% try
		...
		TABLE_CAPTION = cstrcat('Results for ', QUERY, '-', DATASIZE, 'GB, only ncores');
		PLOT_CAPTION = cstrcat('Completion time vs ncores for query ', QUERY, ' with datasize ', DATASIZE, 'GB');
		TABLE_LABEL = cstrcat('tab:', 'only_1_linear_', QUERY, '_', DATASIZE);
		PLOT_LABEL = cstrcat('fig:', 'only_1_linear_', QUERY, '_', DATASIZE);

		% OUTPUT_FOLDER = strcat('output/', QUERY, '_ALL_FEATURES/');
		OUTPUT_FOLDER = strcat('output/', QUERY, '_WEIGHTS_DELETEME/');
		...
		if ismember(1, MODELS_CHOSEN)
			...
			for i=1:length(FEATURES_DESCRIPTIONS)
				fprintf('\n%9.6f -> %s', w(i), FEATURES_DESCRIPTIONS{i});
			end
		...
		if OUTPUT_LATEX
			...
			fprintf(flatex_table, cstrcat('\\begin{table}[H]\n', ...
							...
							'\\begin{tabular}{c | c M{1.4cm} M{2.3cm} M{2.3cm} M{1.8cm}}\n'));
			if TEST_ON_CORES
