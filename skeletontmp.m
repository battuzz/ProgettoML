...
TRAIN_DATA_LOCATION = {'Query R/R1','Query R/R2','Query R/R3','Query R/R4'};
...
if OUTPUT_LATEX
	fprintf(flatex, cstrcat('\\end{tabular}\n', ...
					'\\end{adjustbox}\n', ...
					'\\\\\n', ...
					'\\caption{Previsione del tempo di completamento di ', QUERY, ' facendo training sulle altre query}\n', ...
	...
	% 						'\\caption {Completion time vs Datasize (query ', QUERY, ', solo Datasize)}\n', ...
	% 						'\\end {figure}\n'));	
	fclose(flatex);
end
...
for col = 1:M
	...
	if SAVE_DATA & ismember(13, FEATURES) & (col == M)
		file_location = strrep(strcat(OUTPUT_FOLDER, 'plot_', OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{2}), ' ', '');
		print(OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{1}, file_location);
	end
